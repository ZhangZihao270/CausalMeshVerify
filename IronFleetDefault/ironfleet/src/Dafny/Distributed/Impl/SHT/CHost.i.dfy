...
/************************** AutoMan Translation ************************/
	...
	/** 15 lines of manual code for I1 */
    ...
    method CNextDelegateReal(s: CHost, pkt: CPacket) returns (s':CHost, out:seq<CPacket>)
        ...
        ensures NextDelegate(
			...
			|| HostIgnoringUnParseable(AbstractifyCHostToHost(s), AbstractifyCHostToHost(s'), SeqToSet(AbstractifySeq(out, AbstractifyCPacketToShtPacket)))
		...
		// ensures var (s', out) := CNextDelegateReal(s, pkt); CHostIsValid(s') && (forall i :: i in out ==> CPacketIsValid(i)) && NextDelegateReal(AbstractifyCHostToHost(s), AbstractifyCHostToHost(s'), AbstractifyCPacketToPacket(pkt), AbstractifySet(out, AbstractifyCPacketToPacket))
	{
		...
		out := 
			...
			if pkt.src in s.constants.hostIds { 
				...
				// if ok {			
					// assert AbstractifyCDelegationMapToDelegationMap(delegationMap') == UpdateDelegationMap(AbstractifyCDelegationMapToDelegationMap(s.delegationMap), pkt.msg.m.range, AbstractifyEndPointToNodeIdentity(s.me));
					var h' := 
				...
				// 	assert HostIgnoringUnParseable(AbstractifyCHostToHost(s), AbstractifyCHostToHost(s'), SeqToSet(AbstractifySeq(out, AbstractifyCPacketToShtPacket)));
				// }		
				// var t3 := 
				// 	s.numDelegations + 1; 				
				// (t1, t2, t3) 
