...
//     var nodes := lemma_Propagation(b, i, p);
//     assert |nodes| > 1;
...
//     // find the received propagation
//     // 
//     // lemma_ActionThatSendsPropagationIsReceivePropogateOrReceiveWrite
// }
    ...
    ensures AVersionIsMetOnAllServers(b, i, ios[0].r.msg.key, ios[0].r.msg.meta.vc)
{
    ...
    var nodes := lemma_Propagation(b, i-1, p);
    assert |nodes| > 1;
    assert nodes[0] == p.msg.start;
    assert nodes[|nodes|-2] == p.src;
    ...
    //         ReceiveWrite(b[i-1].servers[idx].s, b[i].servers[idx].s, ios[0].r, ExtractSentPacketsFromIos(ios))
{
    ...
    if ios[0].r.msg.Message_Propagation? {
        lemma_PropagationNotTail(b, i, idx, p.msg.key, p.msg.meta.vc, ios);
        assert AVersionOfAKeyIsMet(b[i].servers[idx].s.icache, b[i].servers[idx].s.ccache, p.msg.key, p.msg.meta.vc);
        var rp := ios[0].r;
        ...
        assert prev_nodes[|prev_nodes|-2] == rp.src;
        assert prev_nodes[|prev_nodes|-1] == rp.dst;
        assert prev_nodes[|prev_nodes|-1] == p.src;
        assert forall j :: 0 <= j < |prev_nodes| - 1 ==> AVersionOfAKeyIsMet(b[i-1].servers[prev_nodes[j]].s.icache, b[i-1].servers[prev_nodes[j]].s.ccache, rp.msg.key, rp.msg.meta.vc);
        ...
        lemma_PropagationAtHead(b, i, idx, p.msg.key, p.msg.meta.vc, ios);
        assert AVersionOfAKeyIsMet(b[i].servers[idx].s.icache, b[i].servers[idx].s.ccache, p.msg.key, p.msg.meta.vc);
...
//     ensures |nodes| > 0
//     ensures nodes[0] == p.msg.start
//     ensures nodes[|nodes|-1] == p.src
//     ensures forall j :: 0 <= j < |nodes| ==> 0 <= nodes[j] < Nodes
//     ensures |nodes| > 1 ==> (forall j :: 0 <= j < |nodes| - 1 ==> NodesAreNext(nodes[j], nodes[j+1]))
//     // ensures p.src in nodes
//     // ensures var n := |nodes|;
//     //         forall j :: 0 <= j < n ==> CalNode(p.msg.start, n) in nodes
//     // requires var sp := ExtractSentPacketsFromIos(ios);
...
//         var rp := ios[0].r;
//         assume rp.dst == idx;
//         assume NodesAreNext(rp.src, rp.dst);
//         assert NodesAreNext(rp.src, p.src);

//         assert rp.msg.Message_Propagation?;
//         assert rp in b[i-1].environment.sentPackets;
//         assert PacketValid(rp);
//         var prev_nodes := lemma_Propagation(b, i-1, rp);
//         // assume rp.src in prev_nodes;
//         assert |prev_nodes| > 0;
//         assert prev_nodes[0] == p.msg.start;
//         assert prev_nodes[|prev_nodes|-1] == rp.src;
//         // assume forall n :: n in prev_nodes ==> 0 <= n < Nodes;
//         assert forall j :: 0 <= j < |prev_nodes| ==> 0 <= prev_nodes[j] < Nodes;
//         assert |prev_nodes| > 1 ==> (forall j :: 0 <= j < |prev_nodes| - 1 ==> NodesAreNext(prev_nodes[j], prev_nodes[j+1]));
//         // assume forall j :: 0 < j <= |prev_nodes| ==> var k := CalNode(p.msg.start, j);
//         //         k in prev_nodes; // && AVersionOfAKeyIsMet(b[i-1].servers[k].s.icache, b[i-1].servers[k].s.ccache, p.msg.key, p.msg.meta.vc);
//         nodes := prev_nodes + [idx];
//         assert nodes[0] == p.msg.start;
//         assert nodes[|nodes|-1] == p.src;
//         assert |nodes| > 1;
//         assert forall j :: 0 <= j < |nodes| ==> 0 <= nodes[j] < Nodes;
        
//         assume (forall j :: 0 <= j < |nodes| - 1 ==> NodesAreNext(nodes[j], nodes[j+1]));
//     } else {
//         lemma_PropagationAtHead(b, i, idx, p.msg.key, p.msg.meta.vc, ios);
//         assert AVersionOfAKeyIsMet(b[i].servers[idx].s.icache, b[i].servers[idx].s.ccache, p.msg.key, p.msg.meta.vc);
//         assert idx == p.msg.start;
...
//         assert p.src in nodes;
//         assert nodes[|nodes|-1] == p.src;
//         assert |nodes| > 1 ==> (forall j :: 0 <= j < |nodes| - 1 ==> NodesAreNext(nodes[j], nodes[j+1]));
//         // assert 0 <= p.msg.start < Nodes;
//         // assert p.msg.start + (n-1) == p.msg.start;
//         // assume p.msg.start % Nodes == p.msg.start;
//         // assert (p.msg.start + (n-1)) % Nodes == p.msg.start;
//         // assert CalNode(p.msg.start, n) in nodes;
//         // assume forall j :: 0 < j <= n ==> var k := CalNode(p.msg.start, j); k in nodes;
//         // assert forall j :: 0 < j <= n ==> var k := CalNode(p.msg.start, j); 
//         //             AVersionOfAKeyIsMet(b[i].servers[k].s.icache, b[i].servers[k].s.ccache, p.msg.key, p.msg.meta.vc);
//         // return;
//     }
//     assert |nodes| > 0;
//     assert nodes[0] == p.msg.start;
...
lemma lemma_PropagationNotTail(
    ...
    requires var sp := ExtractSentPacketsFromIos(ios);
            sp[0].msg.Message_Propagation? && sp[0].msg.meta.vc == vc
    ensures AVersionOfAKeyIsMet(b[i].servers[idx].s.icache, b[i].servers[idx].s.ccache, key, vc)
