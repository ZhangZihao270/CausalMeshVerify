    ...
    ensures AllReadRepliesAreMet(b, i)
{
    ...
    assert forall pp :: pp in b[i-1].environment.sentPackets && pp.msg.Message_Read_Reply? ==> 
        AllVersionsInDepsAreMetOnAllServers(b, i-1, pp.msg.deps_rreply) && AVersionIsMetOnAllServers(b, i-1, pp.msg.key_rreply, pp.msg.vc_rreply);
    ...
    forall pp | pp in b[i].environment.sentPackets && pp.msg.Message_Read_Reply?
        ensures AllVersionsInDepsAreMetOnAllServers(b, i, pp.msg.deps_rreply) && AVersionIsMetOnAllServers(b, i, pp.msg.key_rreply, pp.msg.vc_rreply)
    {
    ...
    ensures VCHappendsBefore(sp[0].msg.vc_rreply, sp[0].msg.pvc_rreply) || VCEq(sp[0].msg.vc_rreply, sp[0].msg.pvc_rreply)
{
    ...
    assert forall k :: k in deps ==> VCHappendsBefore(deps[k], new_pvc) || VCEq(deps[k], new_pvc);
